...
if ( $hasHostnames$ ) {
  ...
  "www.youtube.com"
    ...
    if ( todoIndices.size ) {
        const $scriptletArglistRefs$ = /* 2 */ "-57;1,2";
...
[replaceNodeText,removeNodeText];
    const $scriptletArgs$ = /* 6 */ [
  "script",
  "(function serverContract()",
  "(()=>{if(\"YOUTUBE_PREMIUM_LOGO\"===ytInitialData?.topbar?.desktopTopbarRenderer?.logo?.topbarLogoRenderer?.iconImage?.iconType||location.href.startsWith(\"https://www.youtube.com/tv#/\")||location.href.startsWith(\"https://www.youtube.com/embed/\"))return;const e=ytcfg.data_.INNERTUBE_CONTEXT.client.userAgent,t=t=>{ytcfg.data_.INNERTUBE_CONTEXT.client.userAgent=t?e.replace?.(/(Mozilla\\/5\\.0 \\([^)]+)/,\"$1; \"+t):e},o=[\"channel\",\"lactmilli\"];let r=!1,n=o;document.addEventListener(\"DOMContentLoaded\",(function(){const e=()=>{const e=document.getElementById(\"movie_player\");if(!e||!window.location.href.includes(\"/watch?\"))return void(n=o);const a=e.getPlayerResponse?.(),s=e.getProgressState?.(),i=e.getStatsForNerds?.();let l,c;if(s&&s.duration>0&&(s.loaded<s.duration||s.duration-s.current>1)||a?.videoDetails?.isLive){if(!i?.debug_info?.startsWith?.(\"SSAP, AD\")){const o=a.videoDetails?.videoId,d=a.playerConfig?.playbackStartConfig?.startSeconds??0,y=e.getPlayerStateObject?.()?.isBuffering,u=a.playabilityStatus?.errorScreen,p=JSON.stringify(u?.playerErrorMessageRenderer?.subreason?.runs||u?.playerInterstitialRenderer?.content?.interstitialViewModel?.description?.commandRuns);return void(\"UNPLAYABLE\"===a?.playabilityStatus?.status&&!u?.playerErrorMessageRenderer?.playerCaptchaViewModel&&p?.includes?.(\"WEB_PAGE_TYPE_UNKNOWN\")&&p?.includes?.(\"https://support.google.com/youtube/answer/3037019\")?(n=n.slice(1),n.length>0?t(n[0]):t(\"\"),r=!1,e.loadVideoById(o,d)):0===n.length?(r=!1,t(\"\")):y&&\"0.00 s\"===i?.buffer_health_seconds&&\"0x0\"===i?.resolution&&r?(t(n[0]),r=!1,e.loadVideoById(o,d)):!r&&s.current-d<10&&location.href.includes(\"&list=\")&&\"none\"===e.querySelector(\".ytp-left-controls > .ytp-prev-button\")?.style?.display&&(l=document.querySelector(\"yt-playlist-manager\"),c=l?.getPlaylistData?.(),c&&(l.setPlaylistData?.(c),l.setPlayerPlaybackControlData?.({playlistPanelRenderer:c}))))}s.duration>0&&e.seekTo?.(s.duration)}};e(),new MutationObserver((()=>{e()})).observe(document,{childList:!0,subtree:!0})})),window.Map.prototype.has=new Proxy(window.Map.prototype.has,{apply:(e,t,o)=>{if(\"onSnackbarMessage\"===o?.[0]&&!r){const e=document.getElementById(\"movie_player\");if(!e)return;const t=e.getStatsForNerds?.(),o=e.getPlayerStateObject?.()?.isBuffering,a=e.getPlayerResponse?.()?.playbackTracking?.videostatsPlaybackUrl?.baseUrl;o&&\"0.00 s\"===t?.buffer_health_seconds&&\"0x0\"===t?.resolution&&n.length>0&&(a.includes(\"reloadxhr\")&&(n=n.slice(1)),r=!0)}return Reflect.apply(e,t,o)}});const a={apply:(e,t,o)=>{const r=o[0];return\"function\"==typeof r&&r.toString().includes(\"onAbnormalityDetected\")&&(o[0]=function(){}),Reflect.apply(e,t,o)}};window.Promise.prototype.then=new Proxy(window.Promise.prototype.then,a)})();(function serverContract()",
  "sedCount",
  "1",
  "window,\"fetch\""
];
    const $scriptletArglists$ = /* 2 */ ";0,0,1,2,3,4;1,0,5";
